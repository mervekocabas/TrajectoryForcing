...
class LabelEmbedder(nn.Module):
    ...
    weight_init: str = "scaled_variance"
    init_constant: float = 1.0
    dtype: Any = None  # unused (embedding stays fp32), kept for interface consistency
...
class TokenEmbed(nn.Module):
    """Latent token embedding without patchifying.
    ...
    def setup(self):
        self.num_patches = self.img_size * self.img_size
    ...
    def __call__(self, x):
        ...
        if h * w != self.num_patches:
            raise ValueError(
                f"TokenEmbed expected H*W={self.num_patches}, got {h*w} for shape {(bsz, h, w, c)}"
            )
        x = x.reshape(bsz, h * w, c)
        return self.proj(x)