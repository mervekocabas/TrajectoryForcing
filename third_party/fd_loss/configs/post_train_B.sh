#!/bin/bash
set -euo pipefail
# FD-loss post-training for a pmfDiT_B_16 TF checkpoint.
...
#   Usage  : bash configs/post_train_B.sh <TF_ckpt> [output_dir]
...
#   Output : <output_dir>/pmfDiT_fd/pmfDiT_B_16-fd/tf_checkpoint/<TF_ckpt name> (TF/flax)
...
EXP=pmfDiT_B_16-fd; OUT="$OUT_DIR/pmfDiT_fd/$EXP"
...
torchrun --nnodes=1 --node_rank=0 --master_addr=127.0.0.1 --master_port=29515 --nproc_per_node=8 \
    main_fd.py \
    --project pmfDiT_fd \
    --exp_name pmfDiT_B_16-fd \
    --output_dir "$OUT_DIR" \
    --batch_size 64 \
    --load_from "${INIT_PTH}" \
    --model pmfDiT_B_16 --tokenizer rae_dinov2_b_vitxl \
    --rae_stats_path none \
    --img_size 16 --patch_size 1 --token_channels 768 \
    --noise_scale 1.0 \
    --cfg 1.0 --interval_min 0.0 --interval_max 1.0 \
    --num_sampling_steps 1 \
    --eval_bsz 64 --num_images_for_eval_and_search 50000 \
