...
class TokenEmbed(nn.Module):
    """Per-position linear embedding for latent inputs (no patchify).

    Mirrors pmf_fd_loss/models/embedder.py:TokenEmbed.
    Input:  (B, C, H, W) — torch NCHW. H*W must equal input_size**2.
    Output: (B, H*W, hidden_size).
    """

    def __init__(self, input_size, in_channels, hidden_size, bias=True):
        super().__init__()
        self.img_size = (input_size, input_size)
        self.patch_size = (1, 1)
        self.grid_size = (input_size, input_size)
        self.num_patches = input_size * input_size
        self.proj = TorchLinear(
            in_channels, hidden_size, bias=bias,
    ...
    def forward(self, x):
        ...
        if h * w != self.num_patches:
            raise ValueError(
                f"TokenEmbed expected H*W={self.num_patches}, got {h*w} for shape {(b,c,h,w)}"
            )
